...
int main(int argc, char** argv) {
    ...
    // A unique name per injection. Windows keys loaded modules by path, so a
    // fixed name means LoadLibrary on an already-loaded copy just bumps the
    // reference count and never calls DllMain — the second injection would do
    // nothing at all. A fresh name loads a genuinely new module, which is what
    // makes inject-test-inject work without unloading the previous one or
    // restarting the game.
    char unique[64];
    sprintf_s(unique, sizeof(unique), "bg%08X%04X.dll",
              (unsigned)GetTickCount(), (unsigned)(GetCurrentProcessId() & 0xFFFF));

    std::string dllPath = std::string(tempPath) + unique;
