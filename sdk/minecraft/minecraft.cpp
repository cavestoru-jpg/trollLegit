...
bool sdk::minecraft_client::do_attack()
{
	...
	sdk::camera_data sdk::minecraft_client::get_camera()
	{
		...
		// Get Camera.pos field (1.21.11: getPos() method was removed; read field directly)
		jfieldID camera_get_pos_mid = env->GetFieldID(camera_class, sdk::mappings::camera_pos_field_name, sdk::mappings::camera_pos_field_sig);
		check_jni_exception(env, "GetFieldID for camera_pos");
		...
		// Get position Vec3d via field access
		jobject pos_vec3d = env->GetObjectField(camera, camera_get_pos_mid);
		if (!pos_vec3d) {
			env->DeleteLocalRef(camera_class);
			env->DeleteLocalRef(camera);
			env->DeleteLocalRef(gamerenderer_class);
