...
struct MenuColors {
    c_vec4 layout{ c_col(26, 32, 46, 224).Value };
    c_vec4 child{ c_col(36, 46, 66, 217).Value };
    c_vec4 text{ c_col(170, 195, 225).Value };
    c_vec4 white{ c_col(245, 250, 255).Value };
    c_vec4 accent{ c_col(122, 184, 255).Value };
    c_vec4 black{ c_col(8, 12, 20, 235).Value };
    c_vec4 lightchild{ c_col(50, 64, 90, 204).Value };
    c_vec4 selectable{ c_col(70, 112, 178, 102).Value };
    c_vec4 button{ c_col(76, 128, 215).Value };

    void reset_to_defaults() {
        layout = c_col(26, 32, 46, 224).Value;
        child = c_col(36, 46, 66, 217).Value;
        text = c_col(170, 195, 225).Value;
        white = c_col(245, 250, 255).Value;
        accent = c_col(122, 184, 255).Value;
        black = c_col(8, 12, 20, 235).Value;
        lightchild = c_col(50, 64, 90, 204).Value;
        selectable = c_col(70, 112, 178, 102).Value;
        button = c_col(76, 128, 215).Value;
    }
...
static int g_theme = 3; // 0=Dark, 1=Darker, 2=Void, 3=Light Blue Glass (default)
...
bool Checkbox(const char* name, const char* description, bool* value, c_vec4* color = nullptr) {
    ...
    c_vec2 pos = window->DC.CursorPos;
    float width = (ImGui::GetCurrentContext()->NextItemData.HasFlags & ImGuiNextItemDataFlags_HasWidth) 
...
bool SimpleButton(const char* label, float width = 0.f) {
    ...
    float w = (ImGui::GetCurrentContext()->NextItemData.HasFlags & ImGuiNextItemDataFlags_HasWidth) 
...
static const char* GetModeName(int mode) {
    switch (mode) {
        ...
        // 2 (legacy "Always") is hidden — bind-row picker switches it to Toggle automatically.
...
void ApplyTheme(int theme) {
    // Per-theme accent so light-blue-glass etc. don't keep dark accents.
    c_vec4 target_accent = g_clr.accent;
    switch (theme) {
        case 0:
            ...
            target_accent = c_col(126, 139, 209).Value;
            break;
        case 1:
        ...
        case 2:
            ...
            target_accent = c_col(150, 150, 150).Value;
            break;
        case 3: // Light Blue Glass (default)
            easing(g_clr.layout,     c_col(26, 32, 46, 224).Value, 24.f);
            easing(g_clr.child,      c_col(36, 46, 66, 217).Value, 24.f);
            easing(g_clr.text,       c_col(170, 195, 225).Value, 24.f);
            easing(g_clr.lightchild, c_col(50, 64, 90, 204).Value, 24.f);
            easing(g_clr.selectable, c_col(70, 112, 178, 102).Value, 24.f);
            easing(g_clr.button,     c_col(76, 128, 215).Value, 24.f);
            target_accent = c_col(122, 184, 255).Value;
            break;
    }
    easing(g_clr.accent, target_accent, 24.f);
}
...
// ============================================
// Module registry (GUI metadata)
//
// Each module is represented by a row in the new dropdown layout: name +
// enable checkbox + keybind chip. Right-clicking the row pops up a settings
// popup which calls `render_settings` to render module-specific knobs.
//
// `enabled` / `keybind` / `mode` may be null for special "config" entries
// (e.g. the GUI hotkey itself, which is just a keybind with no enable
// toggle).
// ============================================
...
struct ModuleEntry {
    const char* name;
    const char* category;          // "Combat" / "Movement" / "Visual" / "Utility" / "Macros" / "Config"
    const char* description;
    bool*       enabled;           // may be null for keybind-only Config entries
    int*        keybind;
    int*        mode;
    std::function<void()> render_settings;
};

static const std::vector<const char*> g_categories = {
    "Combat", "Movement", "Visual", "Utility", "Macros", "Config"
};
static int g_selected_category = 0;
// Module currently shown in the side settings panel (index into g_modules).
// -1 means the panel is hidden.
static int g_settings_module_idx = -1;

static std::vector<ModuleEntry> build_modules() {
    return {
        // ---------------- Movement ----------------
        { "Eagle", "Movement", "Auto-sneak near block edges so you can't walk off",
          &globals::eagle_enabled, &globals::eagle_keybind, &globals::eagle_keybind_mode,
          []{
              Slider("Edge distance",
                     "Forward edge-sense distance (blocks). How far ahead of the hitbox we look for air along the movement direction. Lower = engages later.",
                     &globals::eagle_edge_distance, 0.0f, 0.5f, "%.2f");
              Slider("Side sensitivity",
                     "Multiplier for the two probes perpendicular to movement. 1.0 = same as forward, 0.0 = ignore sides entirely.",
                     &globals::eagle_side_sensitivity, 0.0f, 1.0f, "%.2f");
              Slider("Min speed",
                     "Skip detection when player is slower than this (blocks/tick). Stops the module from sneaking while standing still.",
                     &globals::eagle_min_speed, 0.0f, 0.20f, "%.3f");
              Checkbox("Always sneak (debug)",
                       "Bypass the edge detector and keep sneak pressed whenever the module is on",
                       &globals::eagle_always);
              Checkbox("Debug overlay",
                       "Top-left text showing the live edge detector state",
                       &globals::eagle_debug_overlay);
          } },

        // ---------------- Utility ----------------
        { "Teams", "Utility", "Detect teammates by matching leather-armor color (killaura skips them)",
          &globals::teams_enabled, nullptr, nullptr,
          []{
              float tol = static_cast<float>(globals::teams_color_tolerance);
              Slider("Color tolerance",
                     "Max RGB distance (sum of channel deltas) for two colors to count as the same dye",
                     &tol, 0.0f, 200.0f, "%.0f");
              globals::teams_color_tolerance = static_cast<int>(tol);
          } },

        { "HUD", "Visual", "In-game overlay: watermark, keybinds, target HUD, potion list",
          &globals::hud_enabled, nullptr, nullptr,
          []{
              Checkbox("Watermark", "Show 'enhance v...' badge in the top-right", &globals::hud_watermark_enabled);
              Checkbox("Active keybinds", "Bottom-left list of every enabled module", &globals::hud_keybinds_enabled);
              Checkbox("Target HUD", "Top-right name + HP bar of the killaura's current target (no-op until killaura is back online)", &globals::hud_target_hud_enabled);
              Checkbox("Potion list", "Top-left active potion effects (stub — needs JNI side)", &globals::hud_potion_list_enabled);
          } },

        // ---------------- Combat ----------------
        { "AutoClicker", "Combat", "Random-CPS auto-clicker for LMB / RMB",
          &globals::autoclicker_enabled, &globals::autoclicker_keybind, &globals::autoclicker_keybind_mode,
          []{
              Checkbox("LMB enabled", "Auto-click left mouse button", &globals::autoclicker_lmb_enabled);
              if (globals::autoclicker_lmb_enabled)
              {
                  float lo = static_cast<float>(globals::autoclicker_lmb_min_cps);
                  float hi = static_cast<float>(globals::autoclicker_lmb_max_cps);
                  Slider("LMB min CPS", "Lower bound of clicks/sec", &lo, 1.0f, 30.0f, "%.0f");
                  Slider("LMB max CPS", "Upper bound of clicks/sec", &hi, 1.0f, 30.0f, "%.0f");
                  if (hi < lo) hi = lo;
                  globals::autoclicker_lmb_min_cps = static_cast<int>(lo);
                  globals::autoclicker_lmb_max_cps = static_cast<int>(hi);
                  Checkbox("LMB only while held",
                           "Only fire while you physically hold left mouse",
                           &globals::autoclicker_lmb_hold_only);
                  Checkbox("LMB skip on block",
                           "Don't click when the crosshair is on a block (lets you mine normally)",
                           &globals::autoclicker_lmb_skip_on_block);
              }
              ImGui::Separator();
              // ---- RMB ----
              Checkbox("RMB enabled", "Auto-click right mouse button", &globals::autoclicker_rmb_enabled);
              if (globals::autoclicker_rmb_enabled)
              {
                  float lo = static_cast<float>(globals::autoclicker_rmb_min_cps);
                  float hi = static_cast<float>(globals::autoclicker_rmb_max_cps);
                  Slider("RMB min CPS", "Lower bound of clicks/sec", &lo, 1.0f, 30.0f, "%.0f");
                  Slider("RMB max CPS", "Upper bound of clicks/sec", &hi, 1.0f, 30.0f, "%.0f");
                  if (hi < lo) hi = lo;
                  globals::autoclicker_rmb_min_cps = static_cast<int>(lo);
                  globals::autoclicker_rmb_max_cps = static_cast<int>(hi);
                  Checkbox("RMB only while held",
                           "Only fire while you physically hold right mouse",
                           &globals::autoclicker_rmb_hold_only);
                  Checkbox("RMB blocks only",
                           "Only fire when your main hand holds a placeable block",
                           &globals::autoclicker_rmb_blocks_only);
              }
          } },

        { "Killaura", "Combat", "Rotation goes through the aiming manager and is applied across the whole client tick, so movement and the reported look agree.",
          &globals::killaura_enabled, &globals::killaura_keybind, &globals::killaura_keybind_mode,
          []{
              ImGui::TextColored(ImVec4(1.0f, 0.55f, 0.55f, 1.0f),
                                 "Module is currently disabled.");
              ImGui::TextWrapped("Killaura is wired up end-to-end (target selection, "
                                 "4 rotation profiles, JNI hooks, attack-via-hook, "
                                 "movement correction, target ESP, friends, teams, sprint "
                                 "bypass) but does not yet pass Grim's movement-vs-facing "
                                 "simulation checks while sprinting and exhibits packet-"
                                 "order races under load. The fix requires extending the "
                                 "silent rotation hook to wrap ClientPlayerEntity.tick "
                                 "(so travel() sees the fake yaw too) plus a continuous "
                                 "sprint-suppression layer while a target is held.");
              ImGui::TextWrapped("All settings below are preserved — they take effect "
                                 "again the moment the module is unblocked in code.");
              ImGui::Separator();
              // Target Type
              {
                  static std::vector<bool> tt(4);
                  tt[0] = globals::killaura_target_players;
                  tt[1] = globals::killaura_target_mobs;
                  tt[2] = globals::killaura_target_animals;
                  tt[3] = globals::killaura_target_friends;
                  static const std::vector<std::string> tt_names = { "Players", "Mobs", "Animals", "Friends" };
                  MultiDropdown("Target Type", "What to target", &tt, tt_names);
                  globals::killaura_target_players = tt[0];
                  globals::killaura_target_mobs    = tt[1];
                  globals::killaura_target_animals = tt[2];
                  globals::killaura_target_friends = tt[3];
              }

              // Attack Modifiers
              {
                  static std::vector<bool> am(7);
                  am[0] = globals::killaura_only_critical;
                  am[1] = globals::killaura_smart_critical;
                  am[2] = globals::killaura_dynamic_cooldown;
                  am[3] = globals::killaura_break_shield;
                  am[4] = globals::killaura_unpress_shield;
                  am[5] = globals::killaura_no_attack_when_eat;
                  am[6] = globals::killaura_ignore_walls;
                  static const std::vector<std::string> am_names = {
                      "Only Critical", "Smart Critical", "Dynamic Cooldown",
                      "Break Shield", "UnPress Shield", "No Attack When Eat", "Ignore The Walls"
                  };
                  MultiDropdown("Attack Setting", "Attack modifiers", &am, am_names);
                  globals::killaura_only_critical      = am[0];
                  globals::killaura_smart_critical     = am[1];
                  globals::killaura_dynamic_cooldown   = am[2];
                  globals::killaura_break_shield       = am[3];
                  globals::killaura_unpress_shield     = am[4];
                  globals::killaura_no_attack_when_eat = am[5];
                  globals::killaura_ignore_walls       = am[6];
              }

              // Rotation profile
              {
                  static const std::vector<std::string> rt = { "Snap", "Matrix", "Polar", "Linear" };
                  Dropdown("Rotation Type", "Algorithm driving the silent yaw/pitch", &globals::killaura_rotation_type, rt);
              }

              // Move correction
              Checkbox("Move Correction", "Rotate input vector by (real - fake) yaw — required to walk straight while silent rotation is active", &globals::killaura_move_correction_enabled);
              {
                  static const std::vector<std::string> ct = { "Free", "Focused" };
                  Dropdown("Correction Type", "Free = full delta; Focused = clamp ±30°", &globals::killaura_correction_type, ct);
              }

              // Sprint bypass
              {
                  static const std::vector<std::string> sb = { "None", "MineBlaze", "Grim", "Legit" };
                  Dropdown("Sprint Bypass", "Anticheat sprint bypass for crit reset", &globals::killaura_sprint_bypass, sb);
              }
              ...
              // Target ESP
              Checkbox("Target ESP", "Visualize current target", &globals::killaura_esp_enabled);
              if (globals::killaura_esp_enabled)
              {
                  static const std::vector<std::string> esp_modes = { "Cube", "Circle", "Ghosts" };
                  Dropdown("ESP Style", "Visualization style", &globals::killaura_esp_type, esp_modes);
                  if (globals::killaura_esp_type == 2)
                  {
                      Slider("Ghost Speed", "Orbit speed", &globals::killaura_esp_ghost_speed, 0.1f, 5.0f, "%.2f");
                  }
                  ImGui::ColorEdit4("ESP Color", &globals::killaura_esp_color.x);
              }

              // Friends list — minimal in-memory editor.
              {
                  ImGui::Separator();
                  ImGui::TextUnformatted("Friends list");
                  static char friend_buf[64] = {0};
                  ImGui::SetNextItemWidth(180.0f);
                  ImGui::InputText("##killaura_friend_input", friend_buf, sizeof(friend_buf));
                  ImGui::SameLine();
                  if (ImGui::Button("Add##killaura_friend"))
                  {
                      if (friend_buf[0])
                      {
                          std::string name(friend_buf);
                          bool exists = false;
                          for (const auto& s : globals::killaura_friends) if (s == name) { exists = true; break; }
                          if (!exists) globals::killaura_friends.push_back(name);
                          friend_buf[0] = '\0';
                      }
                  }
                  int remove_index = -1;
                  for (size_t i = 0; i < globals::killaura_friends.size(); ++i)
                  {
                      ImGui::PushID(static_cast<int>(i));
                      ImGui::TextUnformatted(globals::killaura_friends[i].c_str());
                      ImGui::SameLine();
                      if (ImGui::SmallButton("X")) remove_index = static_cast<int>(i);
                      ImGui::PopID();
                  }
                  if (remove_index >= 0)
                      globals::killaura_friends.erase(globals::killaura_friends.begin() + remove_index);
              }
          } },

        { "AimAssist", "Combat", "Auto aim at targets",
          &globals::aimassist_enabled, &globals::aimassist_keybind, &globals::aimassist_mode,
          []{
              static std::vector<bool> axes = {true, true};
              axes[0] = globals::aimassist_horizontal;
              axes[1] = globals::aimassist_vertical;
              static const std::vector<std::string> axis_names = {"Horizontal (Yaw)", "Vertical (Pitch)"};
              MultiDropdown("Axes", "Which axes to aim", &axes, axis_names);
              globals::aimassist_horizontal = axes[0];
              globals::aimassist_vertical = axes[1];
              Checkbox("Use Mouse Input", "Move via mouse input", &globals::aimassist_use_mouse_input);
              if (globals::aimassist_use_mouse_input) {
                  Slider("MC Sensitivity", "In-game sensitivity (0-2)", &globals::aimassist_mouse_sensitivity, 0.01f, 2.0f, "%.2f");
              }
              ...
              float max_dist = (float)globals::aimassist_max_distance;
              Slider("Max Distance", "Target range", &max_dist, 1.0f, 50.0f, "%.1f");
              globals::aimassist_max_distance = (double)max_dist;
          } },

        { "Triggerbot", "Combat", "Auto attack on target",
          &globals::triggerbot_enabled, &globals::triggerbot_keybind, &globals::triggerbot_keybind_mode,
          []{
              static const std::vector<std::string> trigger_modes = {"Custom Delay", "Weapon Cooldown", "Combo"};
              Dropdown("Mode", "Timing mode", &globals::triggerbot_mode, trigger_modes);
              Checkbox("Weapon Only", "Only with weapon", &globals::triggerbot_weapon_only);
              if (globals::triggerbot_mode == 0) {
                  float d = (float)globals::triggerbot_delay_ms;
                  Slider("Delay", "Attack delay (ms)", &d, 0.0f, 500.0f, "%.0f ms");
                  globals::triggerbot_delay_ms = (int)d;
              } else if (globals::triggerbot_mode == 2) {
                  float lo = (float)globals::triggerbot_min_delay_ms;
                  float hi = (float)globals::triggerbot_max_delay_ms;
                  Slider("Min Delay", "Minimum delay (ms)", &lo, 0.0f, 1500.0f, "%.0f ms");
                  Slider("Max Delay", "Maximum delay (ms)", &hi, 0.0f, 1500.0f, "%.0f ms");
                  globals::triggerbot_min_delay_ms = (int)lo;
                  globals::triggerbot_max_delay_ms = (int)hi;
              ...
              Checkbox("Hit Select", "Only at full damage", &globals::triggerbot_hit_select);
              static const std::vector<std::string> crit_modes = {"Off", "Crit Only", "Priority Crit"};
              ...
              if (globals::triggerbot_use_shield) {
                  float sd = (float)globals::triggerbot_shield_duration_ms;
                  Slider("Shield Time", "Shield duration (ms)", &sd, 10.0f, 1000.0f, "%.0f ms");
                  globals::triggerbot_shield_duration_ms = (int)sd;
              }
              Checkbox("Check Shield", "Check if target blocking", &globals::triggerbot_check_shield);
              if (globals::triggerbot_check_shield) {
                  static const std::vector<std::string> shield_actions = {"Don't Click", "Spam Click"};
                  Dropdown("Shield Action", "When target blocking", &globals::triggerbot_shield_action, shield_actions);
              }
              Checkbox("Sprint Reset", "Break sprint after hit (crit boost)", &globals::triggerbot_sprint_reset);
              if (globals::triggerbot_sprint_reset) {
                  float t = (float)globals::triggerbot_sprint_reset_ticks;
                  Slider("Reset Ticks", "Ticks to hold sprint off (1 tick = 50ms)", &t, 1.0f, 10.0f, "%.0f");
                  globals::triggerbot_sprint_reset_ticks = (int)t;
              }
          } },

        { "Reach", "Combat", "Extended attack reach",
          &globals::reach_enabled, &globals::reach_keybind, &globals::reach_mode,
          []{
              float r = (float)globals::reach_distance;
              Slider("Distance", "Reach distance", &r, 3.0f, 6.0f, "%.2f");
              globals::reach_distance = (double)r;
          } },

        { "Hitbox", "Combat", "Expand target hitboxes",
          &globals::hitbox_enabled, &globals::hitbox_keybind, &globals::hitbox_mode,
          []{
              float xz = (float)globals::hitbox_expand_width;
              float y  = (float)globals::hitbox_expand_height;
              Slider("X/Z Expand", "Horizontal expansion", &xz, 0.0f, 1.0f, "%.2f");
              Slider("Y Expand",   "Vertical expansion",   &y,  0.0f, 1.0f, "%.2f");
              globals::hitbox_expand_width  = (double)xz;
              globals::hitbox_expand_height = (double)y;
          } },

        { "Shield Breaker", "Combat", "Auto break shields",
          &globals::shield_breaker_enabled, &globals::shield_breaker_keybind, &globals::shield_breaker_keybind_mode,
          []{
              ...
              float d = (float)globals::shield_breaker_delay_ms;
              Slider("Delay", "Break delay (ms)", &d, 0.0f, 200.0f, "%.0f ms");
              globals::shield_breaker_delay_ms = (int)d;
          } },

        { "AutoCrystal", "Combat", "Auto place and break crystals",
          &globals::autocrystal_enabled, &globals::autocrystal_keybind, &globals::autocrystal_mode,
          []{
              float d = (float)globals::autocrystal_delay_ms;
              Slider("Delay", "Place/break delay (ms)", &d, 0.0f, 500.0f, "%.0f ms");
              globals::autocrystal_delay_ms = (int)d;
              Checkbox("Debug", "Show debug logs", &globals::autocrystal_debug_enabled);
          } },

        { "AutoTotem", "Combat", "Auto equip totem in offhand",
          &globals::autototem_enabled, &globals::autototem_keybind, &globals::autototem_mode,
          []{
              Checkbox("Rage Mode", "Different packet sequence", &globals::autototem_rage_mode);
          } },

        { "Stun Slam", "Combat", "Shield break + mace combo",
          &globals::stun_slam_enabled, nullptr, nullptr,
          []{
              Slider("Chance", "Activation chance %", &globals::stun_slam_chance, 0.0f, 100.0f, "%.0f%%");
              float swap = (float)globals::stun_slam_swap_delay_ms;
              float axe  = (float)globals::stun_slam_axe_delay_ms;
              float mace = (float)globals::stun_slam_mace_delay_ms;
              float fall = (float)globals::stun_slam_min_fall;
              Slider("Swap Delay", "Item swap delay", &swap, 0.0f, 50.0f, "%.0f ms");
              Slider("Axe Delay",  "After axe hit",   &axe,  0.0f, 100.0f, "%.0f ms");
              Slider("Mace Delay", "After mace hit",  &mace, 0.0f, 100.0f, "%.0f ms");
              Slider("Min Fall",   "Min fall distance", &fall, 0.5f, 10.0f, "%.1f");
              globals::stun_slam_swap_delay_ms = (int)swap;
              globals::stun_slam_axe_delay_ms  = (int)axe;
              globals::stun_slam_mace_delay_ms = (int)mace;
              globals::stun_slam_min_fall      = (double)fall;
          } },

        // ---------------- Movement ----------------
        { "Auto Mace", "Movement", "Auto mace on fall",
          &globals::mace_enabled, &globals::mace_keybind, &globals::mace_keybind_mode,
          []{
              ...
              float min_fall = (float)globals::mace_min_fall_distance;
              float height   = (float)globals::mace_height_above_target;
              float hx = (float)globals::mace_fall_hitbox_width;
              float hy = (float)globals::mace_fall_hitbox_height;
              Slider("Min Fall",     "Min fall distance",   &min_fall, 0.0f, 20.0f, "%.1f");
              Slider("Height Above", "Height above target", &height,   0.0f, 50.0f, "%.1f");
              Slider("Hitbox X",     "Fall hitbox width",   &hx, 0.0f, 1.0f, "%.2f");
              Slider("Hitbox Y",     "Fall hitbox height",  &hy, 0.0f, 1.0f, "%.2f");
              globals::mace_min_fall_distance   = (double)min_fall;
              globals::mace_height_above_target = (double)height;
              globals::mace_fall_hitbox_width   = (double)hx;
              globals::mace_fall_hitbox_height  = (double)hy;
          } },

        { "S-Tap", "Movement", "Press S on hit",
          &globals::stap_enabled, nullptr, nullptr,
          []{
              float d = (float)globals::stap_duration_ms;
              Slider("S Duration", "How long to press S", &d, 20.0f, 1500.0f, "%.0f ms");
              globals::stap_duration_ms = (int)d;
          } },

        { "W-Tap", "Movement", "Release W on hit",
          &globals::wtap_enabled, nullptr, nullptr,
          []{
              float d = (float)globals::wtap_duration_ms;
              Slider("W Release", "How long to release W", &d, 20.0f, 1500.0f, "%.0f ms");
              globals::wtap_duration_ms = (int)d;
          } },

        { "Auto Jump Reset", "Movement", "Jump when taking damage",
          &globals::autojumpreset_enabled, &globals::autojumpreset_keybind, &globals::autojumpreset_mode,
          []{
              float c = (float)globals::autojumpreset_cooldown_ms;
              Slider("Cooldown", "Jump cooldown (ms)", &c, 0.0f, 1000.0f, "%.0f ms");
              globals::autojumpreset_cooldown_ms = (int)c;
          } },

        { "Sprint", "Movement", "Always sprint",
          &globals::sprint_enabled, nullptr, nullptr, nullptr },

        { "Flight", "Movement", "Enable flight",
          &globals::flight_enabled, nullptr, nullptr, nullptr },

        // ---------------- Visual ----------------
        { "Box ESP", "Visual", "Draw boxes around players",
          &globals::box_enabled, &globals::esp_keybind, &globals::esp_mode,
          []{
              Checkbox("Health Bar", "Show health bars", &globals::esp_health_bar);
          } },

        { "Storage ESP", "Visual", "Highlight chests/shulkers",
          &globals::storage_esp_enabled, &globals::storage_esp_keybind, &globals::storage_esp_mode,
          []{
              Checkbox("Chests",       "Highlight chests",       &globals::storage_esp_chest);
              Checkbox("Ender Chests", "Highlight ender chests", &globals::storage_esp_ender_chest);
              Checkbox("Shulkers",     "Highlight shulkers",     &globals::storage_esp_shulker);
          } },

        // ---------------- Utility ----------------
        { "Pearl Catch", "Utility", "Catch pearl on key press",
          nullptr, &globals::pearl_catch_keybind, &globals::pearl_catch_mode,
          []{
              static const std::vector<std::string> aim_modes = {"Silent (Detected)", "Visible"};
              Dropdown("Aim Mode", "How rotation is applied", &globals::pearl_catch_aim_mode, aim_modes);
          } },

        { "Server Rotation", "Utility", "Silent aim / fake rotation",
          &globals::server_rotation_enabled, nullptr, nullptr,
          []{
              static const std::vector<std::string> rot_modes = {"Look Up", "Look Down", "Look Behind", "Custom"};
              Dropdown("Mode", "Direction to fake", &globals::server_rotation_mode, rot_modes);
              if (globals::server_rotation_mode == 3) {
                  Slider("Yaw",   "Custom yaw",   &globals::server_rotation_custom_yaw,  -180.0f, 180.0f, "%.0f");
                  Slider("Pitch", "Custom pitch", &globals::server_rotation_custom_pitch, -90.0f,  90.0f, "%.0f");
              }
          } },

        // ---------------- Macros ----------------
        { "Anchor Macro", "Macros", "Auto anchor combo",
          &globals::anchor_macro_enabled, &globals::anchor_macro_keybind, &globals::anchor_macro_mode,
          []{
              Checkbox("Explode", "Explode anchor at end", &globals::anchor_macro_break_anchor);
              float swap    = (float)globals::anchor_macro_swap_delay_ms;
              float charge  = (float)globals::anchor_macro_charge_delay_ms;
              float explode = (float)globals::anchor_macro_break_delay_ms;
              Slider("Swap Delay",    "Item swap delay",  &swap,    40.0f, 200.0f, "%.0f ms");
              Slider("Charge Delay",  "Before charging",  &charge,  80.0f, 300.0f, "%.0f ms");
              Slider("Explode Delay", "Before exploding", &explode, 40.0f, 200.0f, "%.0f ms");
              globals::anchor_macro_swap_delay_ms   = (int)swap;
              globals::anchor_macro_charge_delay_ms = (int)charge;
              globals::anchor_macro_break_delay_ms  = (int)explode;
          } },

        { "Auto Anchor", "Macros", "Charge & blow up an anchor you placed",
          &globals::auto_anchor_enabled, &globals::auto_anchor_keybind, &globals::auto_anchor_mode,
          []{
              float swap   = (float)globals::auto_anchor_swap_delay_ms;
              float charge = (float)globals::auto_anchor_charge_delay_ms;
              Slider("Swap Delay",   "Pause between slot swap and click", &swap,   40.0f, 200.0f, "%.0f ms");
              Slider("Charge Delay", "Pause after charging anchor",       &charge, 80.0f, 300.0f, "%.0f ms");
              globals::auto_anchor_swap_delay_ms   = (int)swap;
              globals::auto_anchor_charge_delay_ms = (int)charge;
          } },

        // ---------------- Config (GUI itself + theme) ----------------
        { "GUI Keybind", "Config", "Key to open this menu",
          nullptr, &globals::gui_keybind, &globals::gui_keybind_mode, nullptr },
    };
}

// Lazily built once per process. References to globals stay valid for the
// process lifetime so a static is safe.
static const std::vector<ModuleEntry>& get_modules() {
    static std::vector<ModuleEntry> mods = build_modules();
    return mods;
}

// ============================================
// Centralized bind processor — bind controls the module's *_enabled flag
// directly so it works without the row toggle being on first.
// ============================================
#include "../hooks/Hook.h"

void tick_module_binds() {
    HWND mc = Hook::get_window();
    bool gameplay_active = !globals::show_gui && (!mc || GetForegroundWindow() == mc);

    // Track per-keybind-pointer last state so edge detection survives across
    // GUI open/close — we keep updating `last` even when not acting so we
    // don't see a stale rising edge after the GUI closes.
    static std::unordered_map<int*, bool> last_state;

    const auto& mods = get_modules();
    for (const auto& e : mods) {
        if (!e.enabled || !e.keybind) continue;
        if (*e.keybind == 0) continue;  // no bind set → user controls via toggle

        bool key_now = (GetAsyncKeyState(*e.keybind) & 0x8000) != 0;
        bool& last   = last_state[e.keybind];
        bool press   = key_now && !last;
        bool release = !key_now && last;
        last = key_now;

        if (!gameplay_active) continue;  // update edge tracker but don't act

        int mode = e.mode ? *e.mode : 1;

        // Mode 2 ("Always") means the row toggle is master and the key is
        // ignored — but every module's mode global still initialises to 2, and
        // the only code that ever promoted it to Toggle on assignment lived in
        // the old menu's module row, which is dead since the menu was replaced.
        // The result was that every bind was captured and stored correctly and
        // then silently discarded here. A key that has actually been assigned
        // is an unambiguous request for it to do something, so honour it.
        if (mode == 2) mode = 1;

        if (mode == 0) {
            // Hold: edge-based so a manual click in the GUI stays in effect
            // until the user actually touches the bound key.
            if (press)        *e.enabled = true;
            else if (release) *e.enabled = false;
        } else if (mode == 1) {
            // Toggle: flip on press edge.
            if (press) *e.enabled = !*e.enabled;
        }
    }
}

// Render one module row: enable-checkbox toggle + name + keybind chip.
// Returns true if the row was right-clicked (caller opens the settings popup).
static bool RenderModuleRow(const ModuleEntry& e, int row_index) {
    ImGuiWindow* w = ImGui::GetCurrentWindow();
    if (w->SkipItems) return false;

    ImGui::PushID(row_index);

    c_vec2 row_origin = w->DC.CursorPos;
    float  full = content_avail_x();
    float  h    = s_(28);

    const float toggle_box   = s_(g_elem.checkbox.button_size);
    const float toggle_pad_l = s_(6);

    // ---------- Toggle (LMB) ----------
    ImGui::SetCursorScreenPos(c_vec2(row_origin.x + toggle_pad_l,
                                     row_origin.y + (h - toggle_box) * 0.5f));
    if (e.enabled) {
        if (ImGui::InvisibleButton("##tg", c_vec2(toggle_box, toggle_box))) {
            *e.enabled = !*e.enabled;
        }
    } else {
        ImGui::Dummy(c_vec2(toggle_box, toggle_box));
    }

    // ---------- Body (rest of row: name + RMB area) ----------
    float body_x = row_origin.x + toggle_pad_l + toggle_box + s_(4);
    float body_w = full - (body_x - row_origin.x) - s_(4);
    if (body_w < s_(20)) body_w = s_(20);

    ImGui::SetCursorScreenPos(c_vec2(body_x, row_origin.y));
    bool body_clicked_l = ImGui::InvisibleButton("##body", c_vec2(body_w, h),
                                                 ImGuiButtonFlags_MouseButtonLeft |
                                                 ImGuiButtonFlags_MouseButtonRight |
                                                 ImGuiButtonFlags_MouseButtonMiddle);
    bool body_hovered   = ImGui::IsItemHovered();
    bool body_clicked_r = body_hovered && ImGui::IsMouseClicked(1);
    bool body_clicked_m = body_hovered && ImGui::IsMouseClicked(2);

    // Advance parent cursor past the row.
    ImGui::SetCursorScreenPos(c_vec2(row_origin.x, row_origin.y + h));
    ImGui::Dummy(c_vec2(0, 0));

    // ---------- Drawing ----------
    static std::unordered_map<int, float> row_hover_alpha;
    float& row_alpha = row_hover_alpha[row_index];
    easing(row_alpha, body_hovered ? 1.f : 0.f, 14.f);
    c_rect row_rect(row_origin, row_origin + c_vec2(full, h));
    rect_filled(w->DrawList, row_rect.Min, row_rect.Max,
                get_clr(g_clr.lightchild, 0.32f * row_alpha), s_(4), 0);

    // Toggle box visual.
    if (e.enabled) {
        c_rect tgl(c_vec2(row_origin.x + toggle_pad_l,
                          row_origin.y + (h - toggle_box) * 0.5f),
                   c_vec2(row_origin.x + toggle_pad_l + toggle_box,
                          row_origin.y + (h + toggle_box) * 0.5f));
        static std::unordered_map<int, float> toggle_anim;
        float& a = toggle_anim[row_index];
        easing(a, *e.enabled ? 1.f : 0.f, 24.f);

        rect_filled(w->DrawList, tgl.Min, tgl.Max,
                    get_clr(g_clr.layout), s_(g_elem.checkbox.button_rounding), 0);
        c_vec2 center = tgl.GetCenter();
        c_vec2 hs = tgl.GetSize() * 0.5f * a;
        rect_filled(w->DrawList, center - hs, center + hs,
                    get_clr(g_clr.accent, a), s_(g_elem.checkbox.button_rounding), 0);
        if (a > 0.01f && g_font_icons_8) {
            text_clipped(w->DrawList, g_font_icons_8, tgl.Min, tgl.Max,
                         get_clr(g_clr.black, a), "F", {0.5f, 0.5f});
        }
    }

    // Name label.
    c_vec4 name_col = (e.enabled && *e.enabled) ? g_clr.white :
                      (e.enabled ? g_clr.text : g_clr.white);

    // ---------- MMB quick-bind ----------
    // Middle-click on the row puts it in "waiting for key" mode; the next
    // key/mouse-button press becomes the module's bind.
    ImGuiID quick_id = w->GetID("##quickbind");
    bool quick_waiting = (e.keybind && g_keybind_waiting_id == quick_id);
    if (body_clicked_m && e.keybind) {
        g_keybind_waiting_id = quick_id;
        quick_waiting = true;
    }
    if (quick_waiting) {
        if (GetAsyncKeyState(VK_ESCAPE) & 0x8000) {
            ...
            int captured = 0;
            for (int vk : {VK_MBUTTON, VK_XBUTTON1, VK_XBUTTON2}) {
                if (GetAsyncKeyState(vk) & 0x8000) { captured = vk; break; }
            }
            if (!captured) {
                for (int vk = 0x08; vk <= 0xFE; vk++) {
                    if (vk == VK_ESCAPE || vk == VK_LBUTTON || vk == VK_RBUTTON) continue;
    ...
    // Reserve only the width the bind text actually needs (or none if there is
    // no bind set) so module names like "Shield Breaker" aren't clipped on a
    // narrow column when no bind is showing.
    const char* bind_txt = quick_waiting ? "..."
                         : (e.keybind && *e.keybind != 0) ? GetKeyName(*e.keybind) : nullptr;
    float bind_w = 0.f;
    if (bind_txt) {
        ImGui::PushFont(g_font_inter_11);
        bind_w = ImGui::CalcTextSize(bind_txt).x + s_(6);
        ImGui::PopFont();
    }
    float name_right = row_origin.x + full - bind_w - s_(4);
    text_clipped(w->DrawList, g_font_inter_11,
                 c_vec2(body_x, row_origin.y),
                 c_vec2(name_right, row_origin.y + h),
                 get_clr(name_col), e.name, {0.f, 0.5f});

    // Small bind indicator on the right.
    if (bind_txt) {
        text_clipped(w->DrawList, g_font_inter_11,
                     c_vec2(name_right, row_origin.y),
                     c_vec2(row_origin.x + full - s_(4), row_origin.y + h),
                     get_clr(quick_waiting ? g_clr.white : g_clr.accent),
                     bind_txt, {1.0f, 0.5f});
    }

    // Open settings popup on RMB anywhere in the row body
    // OR on LMB for rows without an enable toggle (e.g. GUI Keybind config row).
    // (Actual rendering is done by the side settings panel — see RenderWolfMenu.)
    // The caller maps `return true` to setting g_settings_module_idx.

    ImGui::PopID();
    return body_clicked_r || (body_clicked_l && !e.enabled);
}
