...
void enhance::modules::autototem::swap_to_offhand_blatant(int slot)
{
	...
	if (slot < 9)
	{
		// Select the slot containing the totem. Shared helper sends
		// UpdateSelectedSlotC2SPacket immediately so the server sees the new
		// slot before the F key-press triggers the offhand swap.
