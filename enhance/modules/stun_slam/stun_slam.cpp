...
void enhance::modules::stun_slam::swap_to_slot(int slot)
{
	...
	// server sees the new slot before our next attack arrives.
...
static bool is_entity_hit_result(jobject hit_result)
{
	...
	// Use the 1.21.x mapping (class_3966). The previous hardcoded class_1298
	// belonged to a much older MC version and silently failed here, which
	// meant stun_slam never recognised that the crosshair was on an entity.
	jclass cls = sdk::classloader::find_class(env, sdk::mappings::entity_hit_result_class_sig);
	if (!cls) return false;

	jboolean is_entity = env->IsInstanceOf(hit_result, cls);
...
static jobject get_entity_from_hit_result(jobject hit_result)
{
	...
	if (!mid) { env->DeleteLocalRef(cls); return nullptr; }
