...
void enhance::modules::reach::run()
{
	if (!hook_initialized)
	{
		// Posted to the client thread rather than attached here: the worker
		// cannot attach without can_suspend, which is routinely unavailable
		// after a re-injection. Throttled because this runs every worker pass
